...
if IS_TRAIN:
    ...
    shared_actor.load_checkpoint()
    shared_target_actor.load_checkpoint()
    ...
    for i_episode in range(n_episode):
        ...
        for i_step in range(n_step_per_episode):
            state_new_all = []
            action_all = []
            action_all_training = np.zeros([n_platoon, n_output], dtype=np.int64)
            # receive observation
            for i in range(n_platoon):
...
print(f"Mean AoI: {np.mean(AoI_total)}")
print(f"Mean Reward: {np.mean(record_reward_global_)}")
print(f"Mean V2I Rate: {np.mean(V2I_total)}")
print(f"Mean V2V Rate: {np.mean(V2V_total)}")
print("=====================================================")
