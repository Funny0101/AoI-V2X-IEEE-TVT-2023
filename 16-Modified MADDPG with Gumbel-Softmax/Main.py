...
def get_state(env, idx):
    """ Get state from the environment (AoI-Enhanced: adds AoI trend and Peak AoI) """
    ...
    # AoI-Enhanced features
    AoI_trend = np.asarray([(env.AoI[idx] - env.AoI_prev[idx]) / (env.time_slow / env.time_fast)])
    Peak_AoI = np.asarray([env.Peak_AoI[idx] / (env.time_slow / env.time_fast)])
    ...
    return np.concatenate((np.reshape(V2I_abs, -1), np.reshape(V2I_fast, -1), np.reshape(V2V_abs, -1),
                           ...
                           V2V_load_remaining, AoI_trend, Peak_AoI), axis=0)
# ------------------------------------------------------------------------------------------------------------------ #
# ------------------------------------------------------------------------------------------------------------------ #
n_input = len(get_state(env=env, idx=0))
n_output = 6  # number of actions ---> channel selection(3), mode selection(2), power(1)
# --------------------------------------------------------------
print("Creating shared actor network ...")
from Classes.networks import ActorNetwork
shared_actor = ActorNetwork(alpha, n_input, A_fc1_dims, A_fc2_dims, n_platoon, n_actions=n_output, name="shared_actor", agent_label="shared")
shared_target_actor = ActorNetwork(alpha, n_input, A_fc1_dims, A_fc2_dims, n_platoon, n_actions=n_output, name="target_shared_actor", agent_label="shared_target")
shared_target_actor.load_state_dict(shared_actor.state_dict())
...
for index_agent in range(n_platoon):
    ...
    agent = Agent(alpha, beta, n_input, tau, n_output, gamma, l_c1_dims, l_c2_dims,
                  ...
                  shared_actor=shared_actor, shared_target_actor=shared_target_actor)
    agents.append(agent)
...
Peak_AoI_total = np.zeros([n_platoon, n_episode], dtype=np.float16)
...
if IS_TRAIN:
    ...
    for i_episode in range(n_episode):
        ...
        env.Peak_AoI = env.AoI.copy()
        env.AoI_prev = env.AoI.copy()
        ...
        for i_step in range(n_step_per_episode):
            state_new_all = []
            action_all = []
            action_all_training = np.zeros([n_platoon, 3], dtype=np.int64)
            # receive observation
            for i in range(n_platoon):
                action = agents[i].choose_action(state_old_all[i])
                action_all.append(action)

                action_all_training[i, 0] = np.argmax(action[0:3])  # chosen RB
                action_all_training[i, 1] = np.argmax(action[3:5])  # Inter/Intra platoon mode
                action_all_training[i, 2] = np.round(np.clip(((action[5]+1)/2) * max_power, 1, max_power))  # power selected by PL
        ...
        Peak_AoI_total[:, i_episode] = env.Peak_AoI.copy()

        if i_episode % 50 == 0:
            global_agent.save_models()
            shared_actor.save_checkpoint()
            shared_target_actor.save_checkpoint()
            for i in range(n_platoon):
    ...
    Peak_AoI_path = os.path.join(current_dir, "model/" + label + '/Peak_AoI.mat')
    ...
    scipy.io.savemat(Peak_AoI_path, {'Peak_AoI': Peak_AoI_total})
