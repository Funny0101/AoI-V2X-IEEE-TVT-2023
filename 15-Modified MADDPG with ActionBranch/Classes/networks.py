...
class ActorNetwork(nn.Module):
    def __init__(self, alpha, input_dims, fc1_dims, fc2_dims, n_agents, n_actions, name, agent_label,
        ...
        # Action branching: three independent heads for RB, mode, power
        self.head_rb = nn.Linear(self.fc2_dims, 1)
        self.head_mode = nn.Linear(self.fc2_dims, 1)
        self.head_power = nn.Linear(self.fc2_dims, 1)
        ...
        f3 = 0.003
        self.head_rb.weight.data.uniform_(-f3, f3)
        self.head_rb.bias.data.uniform_(-f3, f3)
        self.head_mode.weight.data.uniform_(-f3, f3)
        self.head_mode.bias.data.uniform_(-f3, f3)
        self.head_power.weight.data.uniform_(-f3, f3)
        self.head_power.bias.data.uniform_(-f3, f3)
    ...
    def forward(self, state):
        ...
        rb = T.tanh(self.head_rb(x))
        mode = T.tanh(self.head_mode(x))
        power = T.tanh(self.head_power(x))

        x = T.cat([rb, mode, power], dim=-1)
