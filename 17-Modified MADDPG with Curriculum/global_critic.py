...
class Global_Critic():
    ...
    def global_learn(self, agents_nets, state, action, reward_g, reward_t1, reward_t2, state_, terminal):
        ...
        # Zero shared actor gradients ONCE before all agents
        self.agents_networks[0].actor.optimizer.zero_grad()
        ...
        for i in range(self.number_agents):
            ...
            self.agents_networks[i].local_learn(actor_global_loss_, states[:, i * self.number_states:(i + 1) * self.number_states],
                                                ...
                                                accumulate_actor_grad=True)

        # Step shared actor ONCE with accumulated gradients from all agents
        T.nn.utils.clip_grad_norm_(self.agents_networks[0].actor.parameters(), max_norm=1.0)
        self.agents_networks[0].actor.optimizer.step()

        # Update targets: local critics per-agent, shared actor target once
        for i in range(self.number_agents):
            self.agents_networks[i].update_local_critic_targets()
        self.agents_networks[0].update_shared_actor_target()
