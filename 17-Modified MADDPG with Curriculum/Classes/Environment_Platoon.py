...
class Environ:
    ...
    def __init__(self, down_lane, up_lane, left_lane, right_lane, width, height, n_veh, size_platoon, n_RB,
        ...
        self.AoI_prev = np.ones(int(self.n_Veh / self.size_platoon)) * 100
        self.Peak_AoI = np.ones(int(self.n_Veh / self.size_platoon)) * 100
        self.AoI_exponential_alpha = 0.08  # exponential penalty scaling factor
    ...
    def Age_of_Information(self, V2I_rate):
        ...
        n_platoon = int(self.n_Veh / self.size_platoon)
        max_aoi = self.time_slow / self.time_fast

        for i in range(n_platoon):
            self.AoI_prev[i] = self.AoI[i]
            if V2I_rate[i] >= self.V2I_min:
                self.AoI[i] = 1
            else:
                self.AoI[i] += 1
                if self.AoI[i] >= max_aoi:
                    self.AoI[i] = max_aoi
            if self.AoI[i] > self.Peak_AoI[i]:
                self.Peak_AoI[i] = self.AoI[i]
        return self.AoI
    ...
    def act_for_training(self, actions):
        ...
        for i in range(int(self.n_Veh / self.size_platoon)):
            max_aoi = self.time_slow / self.time_fast
            normalized_aoi = self.AoI[i] / max_aoi

            # Curriculum learning: start with linear penalty (alpha=0), slowly transition to quadratic (alpha=1)
            current_ep = getattr(self, 'current_episode', 0)
            alpha = min(1.0, current_ep / 200.0) 

            # Original linear penalty
            linear_penalty = self.AoI[i] / 20.0
            
            # Quadratic AoI penalty: steeper than linear at high AoI
            # At AoI=100: penalty = 5.0 (same as original linear)
            # At AoI=50:  penalty = 1.25 (original was 2.5, so mid-range is less penalized)
            # At AoI=5:   penalty = 0.0125 (similar to original 0.25 but steeper curve overall)
            quad_penalty = 5.0 * normalized_aoi * normalized_aoi
            
            aoi_penalty = (1.0 - alpha) * linear_penalty + alpha * quad_penalty

            # AoI reduction bonus: strong reward for successfully receiving an update
            # We scale the bonus by alpha too, so it doesn't destabilize early training
            aoi_diff = self.AoI[i] - self.AoI_prev[i]
            if aoi_diff < 0:
                aoi_bonus = 2.0 * alpha
            else:
                aoi_bonus = 0.0

            total_aoi_reward = -aoi_penalty + aoi_bonus
            ...
            if action_temp[i, 1] == 0:
                ...
                per_user_task2_reward[i] = (0.05)*self.Revenue_function(C_rate[i], self.V2I_min) \
                                       - 0.5*math.log(action_temp[i, 2],5) + total_aoi_reward
            else:
                per_user_task1_reward[i] = (-4.95) * (Demand[i] / self.V2V_demand_size) - 0.5 * math.log(
                    action_temp[i, 2], 5)

                per_user_task2_reward[i] = (0.05) * self.Revenue_function(C_rate[i], self.V2I_min) + total_aoi_reward
