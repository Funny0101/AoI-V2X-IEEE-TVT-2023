...
class Environ:
    ...
    def act_for_training(self, actions):
        ...
        n_steps = int(self.time_slow / self.time_fast)

        for i in range(int(self.n_Veh / self.size_platoon)):

            # V2V delivery bonus: reward CAM delivery completion, scaled by remaining time
            if Demand[i] <= 0:
                remaining_ratio = self.individual_time_limit[i] / self.time_slow
                v2v_bonus = 3.0 * remaining_ratio
            else:
                v2v_bonus = 0.0

            if action_temp[i, 1] == 0:
                per_user_task1_reward[i] = (-4.95)*(Demand[i] / self.V2V_demand_size) + v2v_bonus
