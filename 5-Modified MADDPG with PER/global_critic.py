...
class Global_Critic():
    ...
    def global_learn(self, agents_nets, state, action, reward_g, reward_t1, reward_t2, state_, terminal,
        ...
        if is_weights is not None:
            weights = T.tensor(is_weights, dtype=T.float).to(self.global_critic1.device).view(-1, 1)
        else:
            weights = T.ones(self.batch_size, 1).to(self.global_critic1.device)
        ...
        td_errors = (target - q1).detach().cpu().numpy().flatten()
        ...
        q1_loss = (weights * (target - q1) ** 2).mean()
        q2_loss = (weights * (target - q2) ** 2).mean()
