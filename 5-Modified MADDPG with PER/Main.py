    ...
    for i_episode in range(n_episode):
        ...
        for i_step in range(n_step_per_episode):
            ...
            if memory.mem_cntr >= batch_size:
                # PER: beta anneals from 0.4 to 1.0 over training
                beta = min(1.0, 0.4 + (i_episode / n_episode) * 0.6)
                states, actions, rewards_g, rewards_t1, rewards_t2, states_, dones, indices, weights = \
                    memory.sample_buffer(batch_size, beta=beta)

                td_errors = global_agent.global_learn(agents, states, actions, rewards_g, rewards_t1, rewards_t2,
                                                      states_, dones, indices=indices, is_weights=weights)
