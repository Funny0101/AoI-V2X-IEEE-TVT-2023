...
if IS_TRAIN:
    ...
    for i_episode in range(n_episode):
        ...
        for i_step in range(n_step_per_episode):
            ...
            # Noise decay: 0.3 -> 0.05 over training (cosine schedule)
            noise_scale = 0.05 + 0.25 * (0.5 * (1 + math.cos(math.pi * i_episode / n_episode)))
            # receive observation
            for i in range(n_platoon):
                action = agents[i].choose_action(state_old_all[i], noise_scale=noise_scale)
