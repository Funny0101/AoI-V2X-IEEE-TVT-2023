...
class Environ:
    ...
    def act_for_training(self, actions):
        ...
        for i in range(int(self.n_Veh / self.size_platoon)):
            # Exponential AoI penalty: penalizes high AoI more aggressively than linear
            max_aoi = self.time_slow / self.time_fast
            normalized_aoi = self.AoI[i] / max_aoi  # [0, 1]
            aoi_penalty = (np.exp(self.AoI_exponential_alpha * self.AoI[i]) - 1) / (np.exp(self.AoI_exponential_alpha * max_aoi) - 1)

            # Peak AoI penalty: extra penalty when current AoI is near the peak
            peak_penalty = 0.3 * (self.Peak_AoI[i] / max_aoi)

            # AoI reduction bonus: reward for successfully reducing AoI
            aoi_diff = self.AoI[i] - self.AoI_prev[i]
            if aoi_diff < 0:  # AoI decreased (update received)
                aoi_bonus = 0.5
            else:
                aoi_bonus = 0.0

            total_aoi_reward = -aoi_penalty - peak_penalty + aoi_bonus
