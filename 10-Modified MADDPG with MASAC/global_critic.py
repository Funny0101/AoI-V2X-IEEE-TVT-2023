...
class Global_Critic():
    def __init__(self, beta, input_dims, tau, n_actions, gamma, C_fc1_dims, C_fc2_dims, C_fc3_dims,
                 batch_size, n_agents, update_actor_interval, noise, entropy_alpha=0.2):
    ...
    def global_learn(self, agents_nets, state, action, reward_g, reward_t1, reward_t2, state_, terminal):
        ...
        for i in range(self.number_agents):
            ...
            target_act_i, _ = self.agents_networks[i].target_actor.sample(obs_i_, deterministic=True)
        ...
        total_log_prob = T.zeros([self.batch_size, 1], device=self.global_critic1.device)
        for i in range(self.number_agents):
            obs_i = states[:, i * self.number_states:(i + 1) * self.number_states]
            act_i, log_prob_i = self.agents_networks[i].actor.sample(obs_i)
            actions_[:, i * self.number_actions:(i + 1) * self.number_actions] = act_i
            total_log_prob += log_prob_i

        q_global = self.global_critic1.forward(states, actions_.to(self.global_critic1.device))
        actor_global_loss = -(q_global - self.entropy_alpha * total_log_prob)
