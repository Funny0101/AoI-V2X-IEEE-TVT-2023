...
class ReplayBuffer():
    def __init__(self, max_size, input_shape, n_actions, n_agents):
        ...
        self.reward_task3 =  np.zeros((self.mem_size, n_agents), dtype=np.float16)
        self.new_state_memory = np.zeros((self.mem_size, input_shape * n_agents), dtype=np.float16)
        self.terminal_memory = np.zeros(self.mem_size, dtype=bool)

    def store_transition(self, state, action, reward_g, reward_t1, reward_t2, reward_t3, state_, done):
        ...
        self.reward_task3[index] = reward_t3
    ...
    def sample_buffer(self, batch_size):
        ...
        rewards_task3 = self.reward_task3[batch]
        states_ = self.new_state_memory[batch]
        dones = self.terminal_memory[batch]

        return states, actions, rewards_g, rewards_task1, rewards_task2, rewards_task3, states_, dones