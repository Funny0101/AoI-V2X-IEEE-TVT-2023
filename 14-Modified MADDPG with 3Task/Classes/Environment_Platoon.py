...
class Environ:
    ...
    def act_for_training(self, actions):
        ...
        per_user_task3_reward = np.zeros(int(self.n_Veh / self.size_platoon))
        ...
        for i in range(int(self.n_Veh / self.size_platoon)):
            ...
            if action_temp[i, 1] == 0:
                # mode 0 (inter/V2I): power penalty goes to task3
                per_user_task1_reward[i] = (-4.95)*(Demand[i] / self.V2V_demand_size)

                per_user_task2_reward[i] = -platoon_AoI[i] / 20

                per_user_task3_reward[i] = (0.05)*self.Revenue_function(C_rate[i], self.V2I_min) \
                                       - 0.5*math.log(action_temp[i, 2],5)
            else:
                # mode 1 (intra/V2V): power penalty goes to task1
        ...
        return per_user_task1_reward, per_user_task2_reward, per_user_task3_reward, global_reward, platoon_AoI, C_rate, \
