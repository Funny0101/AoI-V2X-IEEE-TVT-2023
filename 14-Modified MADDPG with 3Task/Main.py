...
record_reward_t3_ = np.zeros([n_platoon, n_episode], dtype=np.float16)
...
if IS_TRAIN:
    ...
    for i_episode in range(n_episode):
        ...
        record_reward_t3 = np.zeros([n_platoon, n_step_per_episode], dtype=np.float16)
        ...
        for i_step in range(n_step_per_episode):
            ...
            task_1_r, task_2_r, task_3_r, global_reward, platoon_AoI, C_rate, V_rate, Demand_R, V2V_success = \
                env.act_for_training(action_temp)
            for i in range(n_platoon):
                record_reward_t1[i, i_step] = task_1_r[i]
                record_reward_t2[i, i_step] = task_2_r[i]
                record_reward_t3[i, i_step] = task_3_r[i]
            ...
            memory.store_transition(np.asarray(state_old_all).flatten(), np.asarray(action_all).flatten(),
                                    global_reward, task_1_r, task_2_r, task_3_r, np.asarray(state_new_all).flatten(), done)

            # agents take random samples and learn
            if memory.mem_cntr >= batch_size:
                states, actions, rewards_g, rewards_t1, rewards_t2, rewards_t3, states_, dones = memory.sample_buffer(batch_size)

                global_agent.global_learn(agents, states, actions, rewards_g, rewards_t1, rewards_t2, rewards_t3, states_, dones)
        ...
        record_reward_t3_[:, i_episode] = np.mean(record_reward_t3, axis=1)
    ...
    reward_path_t3 = os.path.join(current_dir, "model/" + label + '/reward_t3.mat')
    ...
    scipy.io.savemat(reward_path_t3, {'reward_t3': record_reward_t3_})
