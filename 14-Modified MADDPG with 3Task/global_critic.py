...
class Global_Critic():
    ...
    def global_learn(self, agents_nets, state, action, reward_g, reward_t1, reward_t2, reward_t3, state_, terminal):
        ...
        rewards_t3 = T.tensor(reward_t3, dtype=T.float).to(self.global_critic1.device)
        ...
        for i in range(self.number_agents):
            ...
            self.agents_networks[i].local_learn(actor_global_loss_, states[:, i * self.number_states:(i + 1) * self.number_states],
                                                ...
                                                rewards_t2[:, i], rewards_t3[:, i], states_[:, i * self.number_states:(i + 1) * self.number_states], done)
